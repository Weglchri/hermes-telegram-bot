...
module.exports = {
    ...
    approvedListDisplayer: async function (quoteObject) {
        var stringList = '';
    ...
    pendingListDisplayer: async function (quoteObject) {
        var stringList = '';
        Object.entries(quoteObject).forEach(function([key, value]) {
                // here goes the logic for evaluation which user gets what prefix
                ((prefix = "", key, value) => {
                    stringList = stringList.concat(`${prefix} ${key}: \t ${value.quote} \n`);
                })();
            }
        );
        return stringList;
